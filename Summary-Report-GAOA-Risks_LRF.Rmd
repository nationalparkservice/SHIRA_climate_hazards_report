...
NPVuln.Hazards<-Hazards.lookup$NPVuln.Hazards[c(3,4)]
NPVuln.rename<-Hazards.lookup$NPVuln.rename[c(3,4)]
...
# Combined tables
Combined.data <- left_join(Shira.data[,-c(10)],npvuln.data[,-c(5)],by=c("Park","Name"))
...
g3 <- ggplot() + 
  ...
  ggtitle("Major Climate Risks (SHIRA and NPVuln)") +
  guides(fill="legend") +
  coord_sf()+
...
dset1 <- Combined.data %>% 
 ...
 mutate(across("Riverine Flooding (NFHL)":"Drought", ~ case_when(. == 5 ~ "Very High",
                                             . == 4 ~ "High",
                                             . == 3 ~ "Moderate"))) %>% 
  mutate(across("Summer Drought":"Sea Level or Surge", ~ case_when(. == 1 ~ "Susceptible"))) %>% 
